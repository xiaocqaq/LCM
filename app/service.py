...
async def soft_delete_project(session: AsyncSession, user: User, project: str,
    ...
    for doc in docs:
        doc.deleted_at = _now()
        # 顺序同 soft_delete_document：先 FTS 再 chunk（FTS 行要反查 chunks）
        await dialect.drop_chunk_lexemes(session, DB_BACKEND, doc.id)
        await session.execute(delete(Chunk).where(Chunk.document_id == doc.id))
        await links_mod.clear_supersede_marks(session, doc)
    ...
    await session.commit()
    # 整个项目一次一条 commit，而不是每篇一条：git log 里
    # "delete project: X（12 篇）" 比 12 条独立的 delete 更能说明发生了什么
...
async def empty_trash(session: AsyncSession, user: User) -> dict:
    ...
    chunk 靠 FK ondelete=CASCADE 跟着走，但 SQLite 的 FTS5 是独立虚拟表、
    没有级联，必须在删 chunk 之前显式清掉（drop_chunk_lexemes 内部按后端分流）。
